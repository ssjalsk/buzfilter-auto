...
def generate_posts_json(posts):
    ...
    for p in posts:
        items.append({
            ...
            "tags": p.get("해시태그", ""),
            "date": p.get("날짜", ""),
            "thumbnail": extract_first_image(p.get("본문HTML", "") or ""),
        })
    return _json.dumps(items, ensure_ascii=False, indent=2).encode("utf-8")
...
- 마크다운/설명 출력 금지
- "복합필터 2개"는 "1+1" 구성과 동일한 의미로 처리할 것
...
# ─────────────────────────────────────────────
    ...
    # ─────────────────────────────────────────────
    ALIGO_SHEET_ID = "1mOV-HmlODZaxPohiVFay9_Rnh31-vhcPXF_b-5-EjB0"  # 테스트 시트 (메인으로 사용)
        ...
        try:
            ...
            try:
                ...
                _creds = ServiceAccountCredentials.from_json_keyfile_name(
                    os.path.join(BASE_DIR,'service_account.json'), scope)
            _gc = gspread.authorize(_creds)
            return _gc.open_by_key(ALIGO_SHEET_ID)
        except:
            return None

    # ── 고객 DB 헬퍼 함수들 ─────────────────────────
    def ensure_db_sheet():
        """고객사 DB관리 시트 없으면 생성 + 헤더 기록"""
        _ss = get_aligo_ss()
        if not _ss: return None
        try:
            _ws = _ss.worksheet(DB_SHEET_NAME)
        except:
            _ws = _ss.add_worksheet(DB_SHEET_NAME, rows=500, cols=len(DB_HEADERS))
            _ws.append_row(DB_HEADERS)
        return _ws

    def load_db():
        """고객사 DB 전체 로드 → list of dict"""
        _ws = get_aligo_ws(DB_SHEET_NAME)
        if not _ws: return []
        _vals = _ws.get_all_values()
        if len(_vals) < 2: return []
        _header = _vals[0]
        _rows = []
        for _r in _vals[1:]:
            if not any(_r): continue
            _d = {_header[i]: (_r[i] if i < len(_r) else '') for i in range(len(_header))}
            _rows.append(_d)
        return _rows
    ...
    def save_customer(data: dict, row_idx: int = None):
        ...
        if row_idx is None:
            # 새 고객 ID 부여
            _existing_ids = [r[0] for r in _vals[1:] if r and r[0]]
            _nums = [int(x) for x in _existing_ids if x.isdigit()]
            _new_id = str(max(_nums) + 1) if _nums else "1"
            _row = [
                _new_id,
                data.get("담당자명",""),
                data.get("사업자명",""),
                ...
                data.get("별칭",""),
                data.get("사업자번호",""),
                str(data.get("선충전잔액",0)),
                datetime.now().strftime("%Y-%m-%d"),
            ]
            _ws.append_row(_row)
        else:
            _existing = _vals[row_idx - 1] if row_idx <= len(_vals) else []
            _new_row = [
                _existing[0] if _existing else "",   # 고객ID 유지
                data.get("담당자명",""),
                data.get("사업자명",""),
                data.get("대표자명",""),
                data.get("별칭",""),
                data.get("사업자번호",""),
                str(data.get("선충전잔액",0)),
                _existing[7] if len(_existing)>7 else datetime.now().strftime("%Y-%m-%d"),
            ]
    ...
    def migrate_from_workSheet():
        """업무시트 B열 기존 담당자명 → 고객사 DB 일괄 마이그레이션"""
        _ws_work = get_aligo_ws("업무시트")
        if not _ws_work: return 0, "업무시트 연결 실패"
        _ws_db = ensure_db_sheet()
        if not _ws_db: return 0, "DB시트 연결 실패"

        _all = _ws_work.get_all_values()
        _names = set()
        for _r in _all[1:]:
            if len(_r) >= 2 and _r[1].strip():
                _names.add(_r[1].strip())

        _existing = load_db()
        _exist_names = {_d.get("담당자명","") for _d in _existing}
        _new_names = sorted(_names - _exist_names)

        if not _new_names: return 0, "신규 추가할 고객 없음 (이미 모두 등록됨)"

        _vals = _ws_db.get_all_values()
        _existing_ids = [r[0] for r in _vals[1:] if r and r[0]]
        _nums = [int(x) for x in _existing_ids if x.isdigit()]
        _next_id = max(_nums) + 1 if _nums else 1

        _batch = []
        for _n in _new_names:
            _batch.append([str(_next_id), _n, "", "", "", "", "0", datetime.now().strftime("%Y-%m-%d")])
            _next_id += 1
    ...
    def get_customer_stats(name: str, all_work_rows: list):
        ...
        _misu_amt = 0
        for _r in _rows:
            try: _total_cost += int(float(str(_r[13]).replace(",","") or 0)) if len(_r)>13 else 0
            except: pass
            try: _total_origin += int(float(str(_r[14]).replace(",","") or 0)) if len(_r)>14 else 0
            except: pass
            if len(_r)>6 and _r[6].strip():
                try:
                    _d = _r[6].strip()
                    for _fmt in ["%Y. %m. %d","%Y.%m.%d","%Y-%m-%d"]:
                        try: _dates.append(datetime.strptime(_d, _fmt)); break
                        except: pass
                except: pass
            if len(_r)>4 and _r[4].strip() == "입금확인":
                _misu_count += 1
                try: _misu_amt += int(float(str(_r[13]).replace(",","") or 0)) if len(_r)>13 else 0
                except: pass

        _recent = max(_dates).strftime("%Y.%m.%d") if _dates else "-"
        _avg_days = None
        if len(_dates) >= 2:
            _sorted = sorted(_dates)
            _gaps = [(_sorted[i+1]-_sorted[i]).days for i in range(len(_sorted)-1)]
            _avg_days = round(sum(_gaps)/len(_gaps))

        return {
            "건수": len(_rows),
            "매출": _total_cost,
            "원가": _total_origin,
            "순이익": _total_cost - _total_origin,
            "최근방문": _recent,
            "평균주기": f"{_avg_days}일" if _avg_days else "-",
            "avg_days": _avg_days,
            "미수건수": _misu_count,
            "미수금": _misu_amt,
        }

    def grade_customer(stats: dict) -> str:
        """매출 기준 고객 등급"""
        _m = stats.get("매출", 0)
        if _m >= 5_000_000: return "🏆 VIP"
        if _m >= 1_000_000: return "⭐ 일반"
        if _m > 0:          return "🌱 신규"
        return "⬜ 미거래"

    def parse_biz_cert_with_claude(image_bytes: bytes, filename: str) -> dict:
        """사업자등록증 이미지 → Claude Vision → 정보 추출"""
        _client = get_anthropic_client()
        _ext = filename.rsplit(".",1)[-1].lower()
        _media = {"jpg":"image/jpeg","jpeg":"image/jpeg","png":"image/png","pdf":"application/pdf"}.get(_ext,"image/jpeg")
        ...
        _resp = _client.messages.create(
            ...
            messages=[{
                "role":"user",
                "content":[
                    {"type":"image","source":{"type":"base64","media_type":_media,"data":_b64}},
                    {"type":"text","text":(
                        "이 사업자등록증에서 다음 정보를 JSON으로만 답하세요. 없는 항목은 빈 문자열.\n"
                        '{"사업자명":"","대표자명":"","사업자번호":""}'
                    )}
                ]
            }]
    ...
    def load_all_work_rows():
        _ws = get_aligo_ws("업무시트")
        if not _ws: return []
        return _ws.get_all_values()[1:]  # 헤더 제외

    st.markdown("## 📰 언론 업무 대시보드")
    ln_tab1, ln_tab2, ln_tab3, ln_tab4 = st.tabs(["💸 미수금 현황", "👥 고객 DB", "💳 입금 처리", "🏦 은행 잔고"])
    ...
    with ln_tab2:
        ...
        _col_ref2, _col_mig = st.columns([1,1])
        with _col_ref2:
            if st.button("🔄 새로고침", key="cust_refresh"):
                get_aligo_ws.clear(); load_all_work_rows.clear(); st.rerun()
        with _col_mig:
            if st.button("📥 업무시트 → DB 마이그레이션 (최초 1회)", key="migrate_btn"):
                with st.spinner("마이그레이션 중..."):
                    _cnt, _msg = migrate_from_workSheet()
                if _cnt > 0: st.success(f"✅ {_msg}")
                else: st.info(_msg)
                get_aligo_ws.clear(); load_all_work_rows.clear(); st.rerun()
        ...
        if not _db_rows:
            st.info("고객 DB가 비어있습니다. 위 '마이그레이션' 버튼으로 업무시트 고객을 가져오세요.")
        else:
            # 검색 + 필터
            _c1, _c2 = st.columns([3,1])
            with _c1:
                _search = st.text_input("🔍 고객명 검색", placeholder="담당자명·사업자명·별칭 검색", key="cust_search")
            with _c2:
                _grade_filter = st.selectbox("등급 필터", ["전체","🏆 VIP","⭐ 일반","🌱 신규","⬜ 미거래"], key="grade_filter")

            # 업무 데이터 로드
            _work_rows = load_all_work_rows()
            ...
            for _d in _db_rows:
                ...
                _biz = _d.get("사업자명","")
                _aka = _d.get("별칭","")
                if _search and _search not in f"{_nm}{_biz}{_aka}": continue
                _st  = get_customer_stats(_nm, _work_rows)
                _gr  = grade_customer(_st)
                if _grade_filter != "전체" and _gr != _grade_filter: continue
                _filtered.append((_d, _st, _gr))

            st.caption(f"총 {len(_filtered)}명 표시")

            # 목록 테이블
            _list_data = []
            for _d, _st, _gr in _filtered:
                _avg = _st.get("avg_days")
                _last_dt = _st.get("최근방문","-")
                # D: 입금 패턴 (늦는 고객 감지)
                _pay_pattern = ""
                if _st["미수건수"] > 0:
                    if _st["미수금"] > 0:
                        _pay_pattern = f"⚠️ 미수 {_st['미수건수']}건"
                # C: 미수금 위험 알림 (최근 방문 기준 30일+)
                _misu_warn = ""
                if _st["미수금"] > 0 and _last_dt != "-":
                    try:
                        for _fmt in ["%Y.%m.%d","%Y-%m-%d","%Y. %m. %d"]:
                            try:
                                _dt = datetime.strptime(_last_dt, _fmt)
                                _elapsed = (datetime.now() - _dt).days
                                if _elapsed >= 30:
                                    _misu_warn = f"🔴 {_elapsed}일 경과"
                                break
                            except: pass
                    except: pass
                ...
                _list_data.append({
                    ...
                    "별칭": _d.get("별칭","") or "-",
                    "업무건수": _st["건수"],
                    "매출합계": f"{_st['매출']:,}원" if _st['매출'] else "-",
                    "순이익": f"{_st['순이익']:,}원" if _st['순이익'] else "-",
                    "최근방문": _last_dt,
                    "방문주기": _st.get("평균주기","-"),
            ...
            st.dataframe(_list_df, use_container_width=True, height=300)

            st.markdown("---")

            # 고객 상세 편집
            st.markdown("#### ✏️ 고객 정보 수정 / 사업자등록증 등록")
            ...
            if _sel:
                ...
                _s1.metric("등급", _sel_gr)
                _s2.metric("총 업무", f"{_sel_st['건수']}건")
                _s3.metric("총 매출", f"{_sel_st['매출']:,}원")
                _s4.metric("순이익", f"{_sel_st['순이익']:,}원")
                _s5.metric("방문주기", _sel_st.get("평균주기","-"))

                # B: 고객 등급 설명
                if _sel_gr == "🏆 VIP":
                    st.success("VIP 고객입니다. 우선 관리 대상입니다.")
                elif _sel_gr == "⭐ 일반":
                    st.info("일반 고객입니다.")
                elif _sel_gr == "🌱 신규":
                    st.warning("신규 고객입니다. 재방문 유도가 필요합니다.")

                # C: 미수금 위험 알림
                if _sel_st["미수건수"] > 0:
                    _last_dt2 = _sel_st.get("최근방문","-")
                    _elapsed2 = "-"
                    if _last_dt2 != "-":
                        try:
                            for _fmt in ["%Y.%m.%d","%Y-%m-%d","%Y. %m. %d"]:
                                try:
                                    _dt2 = datetime.strptime(_last_dt2, _fmt)
                                    _elapsed2 = f"{(datetime.now()-_dt2).days}일"
                                    break
                                except: pass
                        except: pass
                    st.error(f"🔴 미수금 {_sel_st['미수금']:,}원 ({_sel_st['미수건수']}건) — 최근 방문 {_elapsed2} 경과")

                # D: 입금 패턴
                if _sel_st.get("avg_days"):
                    _avg_d = _sel_st["avg_days"]
                    if _avg_d <= 7:    _pattern_msg = "✅ 입금 빠른 고객 (평균 7일 이내)"
                    elif _avg_d <= 30: _pattern_msg = "🟡 평균 입금 고객"
                    else:              _pattern_msg = f"🔴 입금 느린 고객 (평균 {_avg_d}일)"
                    st.info(f"💳 방문 패턴: {_pattern_msg}")

                # 재방문 예측 (A 기능)
                if _sel_st.get("avg_days") and _sel_st.get("최근방문","-") != "-":
                    try:
                        for _fmt in ["%Y.%m.%d","%Y-%m-%d","%Y. %m. %d"]:
                            try:
                                _last_d = datetime.strptime(_sel_st["최근방문"], _fmt)
                                from datetime import timedelta
                                _next_est = _last_d + timedelta(days=_sel_st["avg_days"])
                                _days_left = (_next_est - datetime.now()).days
                                if _days_left <= 0:
                                    st.warning(f"📅 예상 재방문일: {_next_est.strftime('%Y.%m.%d')} (이미 {-_days_left}일 지남 — 연락 추천!)")
                                else:
                                    st.caption(f"📅 예상 재방문일: {_next_est.strftime('%Y.%m.%d')} (약 {_days_left}일 후)")
                                break
                            except: pass
                    except: pass
                    ...
                    if _db_ws2:
                        ...
                        for _ri, _rv in enumerate(_all_v):
                            if _rv and _rv[0] == _cid:
                                ...
                                break
                        _ok = save_customer(
                            {"담당자명":_f_name,"사업자명":_f_biz,"대표자명":_f_rep,
                ...
                if _n_sub and _n_name:
                    ...
                    if _ok2: st.success(f"✅ '{_n_name}' 추가 완료!"); get_aligo_ws.clear(); st.rerun()
                    else: st.error("추가 실패")

    # ── 탭3: 입금 처리 ──────────────────────────────────────
    with ln_tab3:
        st.markdown("### 💳 입금 처리")
        st.caption("입금자명으로 고객을 찾아 업무시트 자동 처리")

        if st.button("🔄 새로고침", key="pay_refresh"):
            get_aligo_ws.clear(); load_all_work_rows.clear(); st.rerun()

        _db_rows_pay = load_db()
        _work_rows_pay = load_all_work_rows()

        st.markdown("#### 1️⃣ 입금자명 입력")
        _payer_input = st.text_input("통장에 찍힌 입금자명", placeholder="예: 홍철수, (주)홍컴퍼니", key="payer_input")

        if _payer_input:
            # 고객 매칭: 담당자명, 사업자명, 대표자명, 별칭 전체 탐색
            _candidates = []
            for _d in _db_rows_pay:
                _all_names = [
                    _d.get("담당자명",""),
                    _d.get("사업자명",""),
                    _d.get("대표자명",""),
                ] + [_a.strip() for _a in (_d.get("별칭","") or "").split(",") if _a.strip()]
                _all_names = [_n for _n in _all_names if _n]
                # 포함 매칭 (양방향)
                for _n in _all_names:
                    if _payer_input in _n or _n in _payer_input:
                        _candidates.append((_d, _n))
                        break

            if not _candidates:
                st.warning(f"'{_payer_input}'와 일치하는 고객을 찾지 못했습니다.")
                st.caption("고객 DB에 해당 이름(별칭 포함)을 등록하면 자동 매칭됩니다.")
            else:
                st.success(f"✅ {len(_candidates)}명 후보 발견")
                _cand_options = [f"[ID:{_d.get('고객ID','')}] {_d.get('담당자명','')} (매칭: {_n})" for _d,_n in _candidates]
                _cand_sel = st.selectbox("해당 고객 선택", _cand_options, key="cand_sel")
                _cand_idx = _cand_options.index(_cand_sel)
                _matched_d = _candidates[_cand_idx][0]
                _matched_name = _matched_d.get("담당자명","")

                st.markdown(f"#### 2️⃣ '{_matched_name}' 미수금 건 목록")
                _misu_items = []
                for _ri, _wr in enumerate(_work_rows_pay):
                    if len(_wr) < 14: continue
                    if _wr[1].strip() != _matched_name: continue
                    if _wr[2].strip() != "언론": continue
                    if _wr[4].strip() != "입금확인": continue
                    try: _amt = int(float(str(_wr[13]).replace(",","") or 0))
                    except: _amt = 0
                    _misu_items.append({
                        "row": _ri + 2,
                        "송출일": _wr[6].strip(),
                ...
                if not _misu_items:
                    # 선충전 처리 안내
                    _pre_bal = int(float(_matched_d.get("선충전잔액","0") or 0))
                    st.info(f"현재 미수금 건 없음. 선충전 잔액: {_pre_bal:,}원")
                    st.markdown("##### 💰 선충전 입금 처리")
                    _pre_amt = st.number_input("선충전 금액 (원)", min_value=0, step=10000, key="pre_amt")
                    if st.button("✅ 선충전 잔액 적립", key="pre_add_btn"):
                        # 선충전 잔액 업데이트
                        _db_ws3 = get_aligo_ws(DB_SHEET_NAME)
                        if _db_ws3:
                            _all_v3 = _db_ws3.get_all_values()
                            for _ri3, _rv3 in enumerate(_all_v3):
                                if _rv3 and _rv3[0] == _matched_d.get("고객ID",""):
                                    _new_bal = _pre_bal + _pre_amt
                                    _db_ws3.update_cell(_ri3+1, 7, str(_new_bal))
                                    get_aligo_ws.clear()
                                    st.success(f"✅ 선충전 {_pre_amt:,}원 적립 완료 (잔액: {_new_bal:,}원)")
                                    break
                else:
                    _misu_df = pd.DataFrame([{
                        "송출일":_m["송출일"], "매체사":_m["매체사"],
                        "금액":f"{_m['금액']:,}원", "업무시트행":_m["row"]
                    } for _m in _misu_items])
                    st.dataframe(_misu_df, use_container_width=True)
                    st.markdown(f"**미수 합계: {sum(_m['금액'] for _m in _misu_items):,}원**")

                    st.markdown("##### 처리할 건 선택")
                    _sel_rows = st.multiselect(
                        "입금 처리할 건 선택 (복수 선택 가능)",
                        options=[f"행{_m['row']} — {_m['송출일']} {_m['매체사']} {_m['금액']:,}원" for _m in _misu_items],
                        key="misu_sel"
                    )
                    _pay_amount = st.number_input("실제 입금 금액 (원)", min_value=0, step=10000, key="pay_amount")

                    if st.button("✅ 입금 처리 확정", key="pay_confirm_btn") and _sel_rows:
                        _ws_work2 = get_aligo_ws("업무시트")
                        if _ws_work2:
                            _processed = 0
                            for _sel_r in _sel_rows:
                                _row_num = int(_sel_r.split("행")[1].split(" ")[0])
                                # R열 입금자명, Q열 입금완료, E열 송출완료
                                _ws_work2.update_cell(_row_num, 18, _payer_input)  # R
                                _ws_work2.update_cell(_row_num, 17, "입금완료")   # Q
                                _ws_work2.update_cell(_row_num, 5,  "송출완료")   # E
                                _processed += 1
                            # 선충전 잔액 차감
                            _pre_bal2 = int(float(_matched_d.get("선충전잔액","0") or 0))
                            _remaining = _pay_amount - sum(_m["금액"] for _m in _misu_items if f"행{_m['row']}" in " ".join(_sel_rows))
                            if _remaining > 0 and _pre_bal2 + _remaining > 0:
                                _db_ws4 = get_aligo_ws(DB_SHEET_NAME)
                                if _db_ws4:
                                    _all_v4 = _db_ws4.get_all_values()
                                    for _ri4, _rv4 in enumerate(_all_v4):
                                        if _rv4 and _rv4[0] == _matched_d.get("고객ID",""):
                                            _db_ws4.update_cell(_ri4+1, 7, str(_pre_bal2 + _remaining))
                                            break
                            get_aligo_ws.clear(); load_all_work_rows.clear()
                            st.success(f"✅ {_processed}건 입금 처리 완료!")
                            st.rerun()
                        else:
                            st.error("업무시트 연결 실패")
                ...
                if st.button("🔄 목록 재빌드", help="홈페이지 블로그 목록을 현재 글 기준으로 다시 생성합니다"):
                    ...
                    if _rb_tok and _rb_sid:
                        with st.spinner("목록 재생성 중..."):
                            ...
                            _rb_posts_json = generate_posts_json(_rb_posts)
                            _rb_ok, _rb_msg = deploy_blog_incremental(
                                _rb_tok, _rb_sid, {
                                    "blog/index.html": _rb_idx,
                                    "blog/posts.json": _rb_posts_json,
