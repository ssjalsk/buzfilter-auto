...
# ─── 언론 업무 시트 연결 헬퍼 ───────────────────────────────
ALIGO_MEDIA_SHEET_ID = "1OJkg679B09qvW5hAY_vT35KD0dl5435peGszwv55Fzs"

def _get_aligo_media_ws(tab_name):
    """알리고미디어 메인 업무 시트 탭 연결"""
    try:
        scope = ["https://spreadsheets.google.com/feeds","https://www.googleapis.com/auth/drive"]
        try:
            creds_dict = json.loads(st.secrets["GOOGLE_CREDENTIALS"])
        ...
        return _gc.open_by_key(ALIGO_MEDIA_SHEET_ID).worksheet(tab_name)
    except Exception:
        return None
...
# ─────────────────────────────────────────────
    ...
    with ln_tab1:
        ...
        st.caption("업무시트 E=입금확인 + C=언론 기준 실시간 조회")
        if st.button("🔄 새로고침", key="misu_refresh"):
            if "misu_cache" in st.session_state:
                del st.session_state["misu_cache"]
        if "misu_cache" not in st.session_state:
            _ws_work = _get_aligo_media_ws("업무시트")
            if _ws_work is None:
                st.error("업무시트 연결 실패 — service_account.json 또는 시트 권한 확인")
            else:
                with st.spinner("미수금 목록 불러오는 중..."):
                    st.session_state["misu_cache"] = _ws_work.get_all_values()
        if "misu_cache" in st.session_state:
            _all_v = st.session_state["misu_cache"]
            _misu_rows = []
            for _i, _row in enumerate(_all_v[1:], 2):
                ...
                _media = _row[8].strip()
                _mat   = _row[10].strip()
                try: _cost = int(float(_row[13].replace(",","").strip() or "0"))
                except: _cost = 0
                _s_raw = _row[18].strip() if len(_row) > 18 else ""
                try:
                    _s_clean = _s_raw.replace(",","")
                    _paid = int(float(_s_clean)) if _s_clean and re.match(r"^-?\d+(\.\d+)?$", _s_clean) else 0
                except: _paid = 0
                _remain = max(0, _cost - _paid)
                _url = f"https://docs.google.com/spreadsheets/d/{ALIGO_MEDIA_SHEET_ID}/edit#gid=1675857415&range=A{_i}"
                _misu_rows.append({
                    "고객명": _name, "송출일": _date, "매체사": _media,
                    "소재": _mat, "청구금액": _cost, "입금됨": _paid,
                    "미수잔액": _remain, "VAT포함": round(_remain * 1.1), "링크": _url
                })
            if not _misu_rows:
                st.success("✅ 현재 미수금 없음!")
            else:
                _df_m = pd.DataFrame(_misu_rows)
                _total_m = int(_df_m["미수잔액"].sum())
                _col1, _col2, _col3 = st.columns(3)
                _col1.metric("📋 건수", f"{len(_df_m)}건")
                _col2.metric("💰 미수 합계", f"{_total_m:,}원")
                _col3.metric("💰 VAT포함", f"{round(_total_m * 1.1):,}원")
                st.markdown("---")
                _df_m["이동"] = _df_m["링크"].apply(lambda u: f'<a href="{u}" target="_blank">→ 이동</a>')
                _show_m = _df_m[["고객명","송출일","매체사","소재","청구금액","입금됨","미수잔액","VAT포함","이동"]]
                st.write(_show_m.to_html(escape=False, index=False), unsafe_allow_html=True)
                st.markdown("---")
                st.markdown("#### 고객별 합계")
                _by_c = _df_m.groupby("고객명")["미수잔액"].sum().sort_values(ascending=False).reset_index()
                st.dataframe(_by_c, use_container_width=True)
    ...
    with ln_tab2:
        ...
        if st.button("🔄 새로고침", key="cust_refresh"):
            if "cust_cache" in st.session_state:
                del st.session_state["cust_cache"]
        if "cust_cache" not in st.session_state:
            _ws_cust = _get_aligo_media_ws("고객정보")
            if _ws_cust:
                with st.spinner("고객 DB 불러오는 중..."):
                    st.session_state["cust_cache"] = _ws_cust.get_all_values()
        if "cust_cache" in st.session_state:
            _cd = st.session_state["cust_cache"]
            if len(_cd) >= 4:
                _cdf = pd.DataFrame(_cd[3:], columns=_cd[2])
                _cdf = _cdf[_cdf.iloc[:, 0].str.strip() != ""]
                _srch = st.text_input("🔍 고객명 검색", placeholder="고객명 일부 입력...", key="cust_search")
                if _srch:
                    _cdf = _cdf[_cdf.iloc[:, 0].str.contains(_srch, na=False)]
                st.dataframe(_cdf.head(100), use_container_width=True)
                st.caption(f"총 {len(_cdf)}개 고객사 표시 중")
    ...
    with ln_tab3:
        ...
        if st.button("🔄 새로고침", key="bank_refresh"):
            if "bank_cache" in st.session_state:
                del st.session_state["bank_cache"]
        if "bank_cache" not in st.session_state:
            _ws_sum = _get_aligo_media_ws("종합 정산시트")
            if _ws_sum:
                with st.spinner("잔고 불러오는 중..."):
                    st.session_state["bank_cache"] = _ws_sum.get("O7:T10")
        if "bank_cache" in st.session_state:
            _bv = st.session_state["bank_cache"]
            if _bv and len(_bv) >= 2:
                _bnames = ["우리은행", "카카오뱅크", "IBK기업은행"]
                _brows = []
                for _bi, _br in enumerate(_bv[1:]):
                    if len(_br) < 5: continue
                    _brows.append({
                        "은행": _bnames[_bi] if _bi < 3 else f"은행{_bi + 1}",
                        "총 입금액":  _br[1] if len(_br) > 1 else "",
                        "총 출금액":  _br[2] if len(_br) > 2 else "",
                        "데이터 잔고": _br[3] if len(_br) > 3 else "",
                        "실 잔액":    _br[4] if len(_br) > 4 else "",
                        "현재 잔액":  _br[5] if len(_br) > 5 else "",
                    })
                if _brows:
                    ...
                    _tot_bal = sum(_to_num(r["현재 잔액"]) for r in _brows)
                    st.metric("💰 3개 은행 합산 현재잔액", f"{_tot_bal:,}원")
