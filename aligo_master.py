...
발목 염좌 18""",
    ...
    st.title("🔍 정산 내역 비교")
    st.caption("두 파일을 날짜 + 언론사 기준으로 자동 비교합니다.")
    col1, col2 = st.columns(2)
    with col1:
        st.markdown("**① 플래나 파일** (정산내역서)")
        file_a = st.file_uploader("플래나 파일 업로드", type=["xlsx", "xls"], key="cmp_a")
    with col2:
        st.markdown("**② 우리 파일**")
        file_b = st.file_uploader("우리 파일 업로드", type=["xlsx", "xls"], key="cmp_b")
    ...
    if file_a and file_b:
        if st.button("🔍 비교 시작", type="primary", use_container_width=True):
            ...
            def norm_date(d):
                ...
                return s2[:10] if len(s2) >= 10 else s

            def norm_price(p):
                try:
                    return int(float(str(p).replace(",", "").replace("원", "").strip()))
                except Exception:
                    return 0

            with st.spinner("파일 분석 중..."):
                # 플래나 파일 파싱 (헤더 7행 = pandas header=6)
                df_a = pd.read_excel(file_a, header=6)
                df_a.columns = [str(c).strip() for c in df_a.columns]
                df_a = df_a[pd.to_numeric(df_a.iloc[:, 0], errors="coerce").notna()].copy()

                date_col_a = "송출일" if "송출일" in df_a.columns else df_a.columns[2]
                media_col_a = "언론사" if "언론사" in df_a.columns else df_a.columns[4]
                price_col_a = "단가" if "단가" in df_a.columns else df_a.columns[6]
                client_col_a = "클라이언트명" if "클라이언트명" in df_a.columns else df_a.columns[3]

                df_a["_date"]  = df_a[date_col_a].apply(norm_date)
                df_a["_media"] = df_a[media_col_a].astype(str).str.strip()
                df_a["_price"] = df_a[price_col_a].apply(norm_price)
                df_a["_key"]   = df_a["_date"] + "|" + df_a["_media"]

                # 우리 파일 파싱 (헤더 1행 = pandas header=0)
                df_b = pd.read_excel(file_b, header=0)
                df_b.columns = [str(c).strip() for c in df_b.columns]
                df_b = df_b[pd.to_numeric(df_b.iloc[:, 0], errors="coerce").notna()].copy()

                date_col_b   = "날짜"        if "날짜"        in df_b.columns else df_b.columns[2]
                media_col_b  = "언론사"       if "언론사"       in df_b.columns else df_b.columns[4]
                price_col_b  = "단가"        if "단가"        in df_b.columns else df_b.columns[6]
                client_col_b = "클라이언트명" if "클라이언트명" in df_b.columns else df_b.columns[1]

                df_b["_date"]  = df_b[date_col_b].apply(norm_date)
                df_b["_media"] = df_b[media_col_b].astype(str).str.strip()
                df_b["_price"] = df_b[price_col_b].apply(norm_price)
                df_b["_key"]   = df_b["_date"] + "|" + df_b["_media"]

                # 우리 파일 multiset lookup
                b_lookup = {}
                for _, row in df_b.iterrows():
                    ...
                    b_lookup[k].append({
                        "price": row["_price"],
                        "client": str(row.get(client_col_b, "")),
                        "used": False,
                    })
                ...
                for _, row in df_a.iterrows():
                    k        = row["_key"]
                    client_a = str(row.get(client_col_a, ""))
                    price_a  = row["_price"]
                    date_a   = row["_date"]
                    media_a  = row["_media"]
                    ...
                    if k in b_lookup:
                        matched = False
                        # 단가까지 일치하는 항목 우선
                        for item in b_lookup[k]:
                            if not item["used"] and item["price"] == price_a:
                                item["used"] = True
                                results.append({
                                    "날짜": date_a, "언론사": media_a,
                                    "플래나_클라이언트": client_a, "우리_클라이언트": item["client"],
                                    "플래나_단가": price_a, "우리_단가": price_a,
                                    "상태": "✅ 정상",
                                })
                                matched = True
                                break
                            ...
                            for item in b_lookup[k]:
                                if not item["used"]:
                                    ...
                                    results.append({
                                        "날짜": date_a, "언론사": media_a,
                                        "플래나_클라이언트": client_a, "우리_클라이언트": item["client"],
                                        "플래나_단가": price_a, "우리_단가": item["price"],
                                        "상태": "💰 금액 불일치",
                                    })
                                    matched = True
                                    break
                        if not matched:
                            results.append({
                                "날짜": date_a, "언론사": media_a,
                                "플래나_클라이언트": client_a, "우리_클라이언트": "",
                                "플래나_단가": price_a, "우리_단가": "",
                                "상태": "❌ 우리 파일 누락",
                            })
                    else:
                        results.append({
                ...
                # 우리 파일에만 남은 항목
                for k, items in b_lookup.items():
                    for item in items:
                        if not item["used"]:
                            date_p, media_p = k.split("|", 1)
                            results.append({
                                "날짜": date_p, "언론사": media_p,
                                "플래나_클라이언트": "", "우리_클라이언트": item["client"],
                                "플래나_단가": "", "우리_단가": item["price"],
                                "상태": "⚠️ 플래나 파일 누락",
                            })

                result_df = pd.DataFrame(results).sort_values("날짜").reset_index(drop=True)

            # 요약 지표
            ok     = (result_df["상태"] == "✅ 정상").sum()
            miss_b = (result_df["상태"] == "❌ 우리 파일 누락").sum()
            miss_a = (result_df["상태"] == "⚠️ 플래나 파일 누락").sum()
            diff   = (result_df["상태"] == "💰 금액 불일치").sum()

            c1, c2, c3, c4 = st.columns(4)
            c1.metric("✅ 정상 매칭",    f"{ok}건")
            c2.metric("❌ 우리 파일 누락", f"{miss_b}건")
            c3.metric("⚠️ 플래나 누락",   f"{miss_a}건")
            c4.metric("💰 금액 불일치",   f"{diff}건")

            # 불일치 항목 표시
            issues = result_df[result_df["상태"] != "✅ 정상"]
            if len(issues) > 0:
                st.markdown(f"### ⚠️ 불일치 항목 {len(issues)}건")
                st.dataframe(issues, use_container_width=True)
            else:
                st.success("🎉 모든 항목이 정상 매칭됩니다!")
            ...
            buf = io.BytesIO()
            with pd.ExcelWriter(buf, engine="openpyxl") as writer:
                result_df.to_excel(writer, index=False, sheet_name="비교결과")
                ws_cmp = writer.sheets["비교결과"]
                fills = {
                    "✅ 정상":          PatternFill("solid", start_color="C6EFCE", end_color="C6EFCE"),
                    "❌ 우리 파일 누락": PatternFill("solid", start_color="FFC7CE", end_color="FFC7CE"),
                    "⚠️ 플래나 파일 누락": PatternFill("solid", start_color="FFEB9C", end_color="FFEB9C"),
                    "💰 금액 불일치":   PatternFill("solid", start_color="FFFACD", end_color="FFFACD"),
                }
                status_col_idx = list(result_df.columns).index("상태") + 1
                for row_idx in range(2, len(result_df) + 2):
                    status = ws_cmp.cell(row=row_idx, column=status_col_idx).value
                    fill = fills.get(status)
                    if fill:
                        for col in range(1, len(result_df.columns) + 1):
                            ws_cmp.cell(row=row_idx, column=col).fill = fill
                for col in ws_cmp.columns:
                    max_len = max((len(str(c.value or "")) for c in col), default=10)
                    ws_cmp.column_dimensions[col[0].column_letter].width = min(max_len + 4, 40)
            buf.seek(0)
            st.download_button(
                "⬇️ 비교 결과 엑셀 다운로드",
                data=buf,
                file_name=f"정산비교_{datetime.now().strftime('%Y%m%d')}.xlsx",
                mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                type="primary",
                use_container_width=True,
            )
